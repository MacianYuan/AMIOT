        ...
        if (res == 0) {
            ...
            if (FD_ISSET(network_handle->fd, &recv_sets)) {
                ...
                if (recv_res == 0) {
                    ...
                    if ((uint32_t)recv_bytes == len) {
                        break;
                    }
                }
            }
        }
    } while (((timenow_ms - timestart_ms) < (uint64_t)timeout_ms) && ((uint32_t)recv_bytes < len));
        ...
        if (res == 0) {
            ...
            if (FD_ISSET(network_handle->fd, &send_sets)) {
                ...
                if (send_res == 0) {
                    ...
                    send_bytes += send_res;
                    if ((uint32_t)send_bytes == len) {
                        break;
                    }
                }
            }
        }
    } while (((timenow_ms - timestart_ms) < (uint64_t)timeout_ms) && ((uint32_t)send_bytes < len));
