...
#include "LinkSDK/core/utils/core_string.h"
#include "LinkSDK/core/utils/core_log.h"
#include "LinkSDK/core/utils/core_global.h"
#include "LinkSDK/core/utils/core_mqtt.h"
...
int32_t aiot_ntp_deinit(void **handle)
{
    ...
    deinit_timeout_ms = ntp_handle->deinit_timeout_ms;
    do {
        if (ntp_handle->exec_count == 0) {
            break;
        }
        ntp_handle->sysdep->core_sysdep_sleep(NTP_DEINIT_INTERVAL_MS);
    } while ((deinit_timeout_ms > NTP_DEINIT_INTERVAL_MS) && (deinit_timeout_ms - NTP_DEINIT_INTERVAL_MS > 0));
