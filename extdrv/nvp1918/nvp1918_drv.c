...
static int __init vdec_module_init(void)
{
	...
	*(volatile unsigned int*)(IO_ADDRESS(0x20160000 + 0x10)) = (1<<2);
	...
	printk("\r\n[david] Init NVP1918 driver start!!!");
	if(vdec_cnt <= 2)
	{
		for(i=0;i<vdec_cnt;i++)
		{
			...
			{
				printk("NVP1918C  Device (0x%x) ID OK... %x\n", vdec_slave_addr[i], chip_id[i]);
			}
		}
	}
	else
	{
		printk("\r\n[david] We have error vdec_cnt num,this num Max Count is 2!!!!");
	}
#ifdef NVP1118B
	nvp1918c_720h_nt();
#else
	//nvp1918_960h_pal();  // 3520D not support 8 chn 960H.  
	nvp1918c_720h_pal();
#endif
	printk("\r\n[david] Init driver end!");
	return 0;
}
