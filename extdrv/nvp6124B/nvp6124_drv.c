...
#if 0   // 2102/10/11 by lkf72840 把之前的 && 运算符 修改为 &  , 解决 I2C 下 9022 无输出的问题
...
static int __init nvp6124_module_init(void)
{
	...
	{
		nvp6124_set_chnmode(ch, PAL, NVP6124_VI_720P_2530);   //默认设置为720P模式.
	}
	for(chip=0;chip<nvp6124_cnt;chip++)
	{
		...
		{
			nvp6124b_set_portmode(chip, 0, NVP6124_OUTMODE_2MUX_MIX, 0);  //设置nvp6124b 2个port的输出模式
			nvp6124b_set_portmode(chip, 1, NVP6124_OUTMODE_2MUX_MIX, 1);
		}
	}
	
	audio_init(1,1,4,0,0);
